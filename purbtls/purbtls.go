...
//DATALEN is the length required for each entry point
const DATALEN = 24
...
//not using buffers correctly TODO check buffers
...
func Server(c net.Conn, conf *Config) *PurbConn {
	...
	for {
		...
		if l > 0 {
			...
			_, _ = purb.AttemptDecode(&entry, entry.Suite, entry.PriKey, KEYPOS,
				checkPurb, buf, random.Stream)
	...
	//Pad the message before encrypting as it is much cheaper TODO ?
	padLen := padding.PaddingLength(uint64(len(m)), uint64(AEADLEN))
	...
	ok := padding.CheckPadding(uint64(len(cipher)))
	if !ok {
...
func Client(c net.Conn, conf *Config) *PurbConn {
	...
	purbHeader, _ := purb.GenPurb(conf.keys, KEYPOS, nil, nil, nil, false)
	//Pad the header
	paddingLen := padding.PaddingLength(uint64(len(purbHeader)), 0)
	...
	//Check length
	padRes := padding.CheckPadding(uint64(len(paddedMsg)))
	if !padRes {
...
func (conn *PurbConn) createRecordDefault(msg []byte, nextLen int) record {
	...
	if len(m) < DEFAULTLEN {
		...
		padLen := padding.PaddingLength(uint64(len(m)), AEADLEN)
