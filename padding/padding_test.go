...
func TestAESGCM(t *testing.T) {
	...
	fmt.Println("Padding needed(overhead): ", p)
	fmt.Println("Increase in message size caused by overhead (padding+len+oh)/len", ((float64(p+24) + float64(len(pt))) / float64(len(pt))))
	...
	ct2 := aesgcm.Seal(nil, nonce, pt, ad)
	//check if our ct2 is correctly padded
	if CheckZeroBits(ct2) {
		fmt.Println("The ciphertext is of a correct length")
		fmt.Println("Length is ", len(ct2))
		fmt.Println(strconv.FormatUint(uint64(len(ct2)), 2))
		fmt.Println(strconv.FormatUint(uint64(len(pt1)), 2))

	}
	//decrypt the ct
	pt, _ = aesgcm.Open(nil, nonce, ct2, ad)
	//unpad the pt
	pt = UnPadGeneric(pt)
